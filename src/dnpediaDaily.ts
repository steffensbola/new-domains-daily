...
export const updateDnPediaDaily = async () => {
  ...
  days.forEach((day) => {
    const tlds = allTlds.filter(
      (t) => t.current_add_count > 0 && t.last_processed === day
    );
    tlds.forEach((tld) => {
  ...
  await Promise.allSettled(promises).then((results) => {
    ...
    saveToFile(
      JSON.stringify(db),
      "data/dnpedia/" + today.toISO().split("T")[0] + ".json"
    );
    saveToFile(
      JSON.stringify(error),
      "data/dnpedia/" + today.toISO().split("T")[0] + ".error.txt"
    );
    saveToFile(
      JSON.stringify(success),
      "data/dnpedia/" + today.toISO().split("T")[0] + ".success.txt"
    );
  });
};
